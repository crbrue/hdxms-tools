...
def main(argv=None):
    ...
    p.add_argument('--cartoon-trans',type=float,default=0.0); p.add_argument('--protein-only',action='store_true'); p.add_argument('--out-png'); p.add_argument('--session'); p.add_argument('--out-bfactor-pdb'); p.add_argument('--out-chimerax')
    ...
    if args.command=='run':
        ...
        cmd=[args.pymol,'-cq','-r',str(module),'--',args.structure,args.csv,'--value-col',args.value_col,'--offset',str(args.offset),'--mincov',str(args.mincov),'--cartoon-trans',str(args.cartoon_trans)]
        for flag,value in [('--chain-filter',args.chain_filter),('--offset-per-chain',args.offset_per_chain),('--out-png',args.out_png),('--session',args.session),('--out-bfactor-pdb',args.out_bfactor_pdb),('--out-chimerax',args.out_chimerax)]:
