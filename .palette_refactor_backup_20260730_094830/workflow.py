...
MASS_COL = "Peptide monoisotopic mass (uncharged)"
COMPARISON_KEYS = KEYS + [MASS_COL]
...
def _pct_stats(frames: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    for i, df in enumerate(frames, 1):
        ...
        peptide_ids = (
            df["Peptide ID"]
            if "Peptide ID" in df.columns
            else pd.Series([None] * len(df), index=df.index)
        )
        part[MASS_COL] = [
            peptide_monoisotopic_mass(seq, charge, peptide_id)
            for seq, charge, peptide_id in zip(df["Sequence"], df["Charge"], peptide_ids)
        ]
        part["replicate"] = i
        rows.append(part)
    all_df = pd.concat(rows, ignore_index=True)
    return (all_df.groupby(COMPARISON_KEYS, as_index=False)
            .agg(avg_pctD=("pctD", "mean"), sd_pctD=("pctD", "std"), n=("pctD", "count"))
            .sort_values(["Start", "End", "Charge", MASS_COL]).reset_index(drop=True))


def _comparison_identity_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Return normalized peptide identity columns used to pair comparison rows."""
    missing = [column for column in COMPARISON_KEYS if column not in df.columns]
    if missing:
        raise KeyError(f"Comparison statistics are missing identity columns: {missing}")

    identity = df[COMPARISON_KEYS].copy()
    identity[MASS_COL] = pd.to_numeric(identity[MASS_COL], errors="raise").round(6)
    return identity


def _merge_comparison_stats(
    ...
    """Compare only common, exactly matching peptide identities.

    Rows are paired on Start, End, Sequence, Charge, and calculated monoisotopic
    mass. Peptides present in only one dataset are excluded. The validated
    one-to-one merge prevents subtraction between mismatched peptide identities.
    """
    sa = stats_a.copy()
    sb = stats_b.copy()
    sa[COMPARISON_KEYS] = _comparison_identity_frame(sa)
    sb[COMPARISON_KEYS] = _comparison_identity_frame(sb)

    sa = sa.rename(columns={
        "avg_pctD": f"avg_pctD_{dataset_a}",
        "sd_pctD": f"sd_pctD_{dataset_a}",
        "n": f"n_{dataset_a}",
    })
    sb = sb.rename(columns={
        "avg_pctD": f"avg_pctD_{dataset_b}",
        "sd_pctD": f"sd_pctD_{dataset_b}",
        "n": f"n_{dataset_b}",
    })
    merged = pd.merge(
        sa,
        ...
        how="inner",
        validate="one_to_one",
    ).sort_values(["Start", "End", "Charge", MASS_COL])
    if merged.empty:
        raise ValueError(
            f"Comparison {comparison_name!r} ({mode}) has no common peptide identities "
            f"between {dataset_a!r} and {dataset_b!r}."
        )
    merged["pctD_diff"] = merged[f"avg_pctD_{dataset_a}"] - merged[f"avg_pctD_{dataset_b}"]
...
def run_manifest(manifest_path: str | Path) -> Path:
    ...
    stats_by_mode: dict[str, dict[str, pd.DataFrame]] = {"no_be": {}, "be": {}}
    ...
    for cond, paths in condition_paths.items():
        ...
        stats_by_mode["no_be"][cond] = _pct_stats(raw_frames)
        stats_by_mode["be"][cond] = _pct_stats([selected[p] for p in paths])
    ...
    for comp in cfg.get("comparisons", []):
        ...
        if a not in stats_by_mode["no_be"] or b not in stats_by_mode["no_be"]:
        ...
        for mode, mode_stats in stats_by_mode.items():
            cdir = comparisons_dir / name / mode
            for sub in ["tables", "figures", "illustrator"]:
                (cdir / sub).mkdir(parents=True, exist_ok=True)

            merged = _merge_comparison_stats(
                mode_stats[a],
                mode_stats[b],
                dataset_a=a,
                dataset_b=b,
            ...
            merged["normalization"] = "none" if mode == "no_be" else selected_label

            pct_path = cdir / "tables" / "pctD_summary.csv"
            merged.to_csv(pct_path, index=False)
            ...
            consensus_from_csv(
                ...
                stat=cfg.get("stat", "mean"),
                mincov=int(cfg.get("mincov", 1)),
            ...
            for z in cfg.get("zoom_regions", []):
                export_consensus_zoom_manual_fixed_size(
                    ...
                    int(z["start"]),
                    int(z["end"]),
                    cdir / "illustrator" / f"consensus_{z['name']}_{z['start']}-{z['end']}.svg",
                )
            plot_aligned_diff(
                pct_path,
                cdir / "figures" / "consensus_domain_diffmap.png",
                offset=int(cfg.get("offset", 0)),
                stat=cfg.get("stat", "mean"),
                mincov=int(cfg.get("mincov", 1)),
                smooth=int(cfg.get("smooth", 0)),
                consensus_output=consensus_path,
                title=comp.get("title", name),
                dpi=dpi,
            )
            consensus = pd.read_csv(consensus_path)
            consensus["analysis_mode"] = mode
            consensus["normalization"] = "none" if mode == "no_be" else selected_label
            consensus.to_csv(consensus_path, index=False)

            structure_value = comp.get("structure")
            if structure_value:
                structure_path = _resolve(root, str(structure_value))
                if not structure_path.exists():
                    raise FileNotFoundError(structure_path)
                structure_dir = cdir / "structure"
                structure_dir.mkdir(parents=True, exist_ok=True)
                pymol_module = Path(__file__).with_name("pymol_script.py")
                cmd = [
                    ...
                    str(consensus_path),
                    "--value-col", "pctD_diff",
                    "--offset", str(int(comp.get("offset", cfg.get("offset", 0)))),
                    "--mincov", str(int(comp.get("mincov", cfg.get("mincov", 1)))),
                    "--cartoon-trans", str(float(comp.get("cartoon_trans", 0.0))),
                ...
                if comp.get("offset_per_chain"):
                    cmd += ["--offset-per-chain", str(comp["offset_per_chain"])]
                if bool(comp.get("protein_only", False)):
                    cmd.append("--protein-only")
                try:
                    ...
                    warnings.append({
                        ...
                        "comparison": name,
                        "mode": mode,
                        "message": f"PyMOL executable not found; skipped structure rendering: {cmd[0]}",
                    })
                    ...
                    warnings.append({
                        ...
                        "comparison": name,
                        "mode": mode,
                        "message": f"PyMOL structure rendering failed with exit code {exc.returncode}",
                    })
